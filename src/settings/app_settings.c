...
#define SETTINGS_RECORD_SLOT_A_KEY "cfg8_a"
#define SETTINGS_RECORD_SLOT_B_KEY "cfg8_b"
                                ...
                                app_settings_t *settings)
{
    ...
    if (error == ESP_OK) {
        bool manual_saving_requested = false;
        if (app_manual_saving_from_legacy_power(
                1U, value_u8, &manual_saving_requested)) {
    ...
    if (error == ESP_OK && value_u8 <= 100U) {
        ...
        settings->alarm_volume = value_u8;
    } else if (!recoverable_field_error(error) && error != ESP_OK) {
        return error;
    }
...
static const char *schema5_record_slot_key(settings_record_slot_t slot)
{
    if (slot == SETTINGS_RECORD_SLOT_A) {
        return SETTINGS_SCHEMA5_RECORD_SLOT_A_KEY;
    }
    if (slot == SETTINGS_RECORD_SLOT_B) {
        return SETTINGS_SCHEMA5_RECORD_SLOT_B_KEY;
    }
    return NULL;
}
...
{
    if (slot == SETTINGS_RECORD_SLOT_A) {
        return SETTINGS_SCHEMA3_RECORD_SLOT_A_KEY;
    }
    if (slot == SETTINGS_RECORD_SLOT_B) {
...
static esp_err_t read_schema5_record_slot(
    nvs_handle_t handle, settings_record_slot_t slot,
    settings_record_t *record, bool *valid)
{
    const char *key = schema5_record_slot_key(slot);
    if (key == NULL || record == NULL || valid == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    *valid = false;
    ...
    if (encoded_size != SETTINGS_RECORD_SCHEMA5_ENCODED_SIZE) {
        return ESP_OK;
    }
    ...
    *valid = settings_record_decode_schema5(encoded, encoded_size,
                                            record);
    return ESP_OK;
}

static esp_err_t load_schema5_fields(nvs_handle_t handle,
                                     app_settings_t *settings,
                                     bool *found)
{
    ...
    *found = false;
    ...
    esp_err_t error = read_schema5_record_slot(
        handle, SETTINGS_RECORD_SLOT_A, &slot_a, &slot_a_valid);
    if (error == ESP_OK) {
        error = read_schema5_record_slot(
...
static esp_err_t read_schema3_record_slot(
    ...
    settings_record_t *record, bool *valid)
{
    const char *key = schema3_record_slot_key(slot);
    if (key == NULL || record == NULL || valid == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    *valid = false;
    ...
    if (encoded_size != SETTINGS_RECORD_SCHEMA3_ENCODED_SIZE) {
        return ESP_OK;
    }
    ...
    *valid = settings_record_decode_schema3(encoded, encoded_size, record);
    return ESP_OK;
}

static esp_err_t load_schema3_fields(nvs_handle_t handle,
                                     app_settings_t *settings,
                                     bool *found)
{
    if (settings == NULL || found == NULL) {
    ...
    *found = false;
    ...
    bool slot_a_valid = false;
    bool slot_b_valid = false;
    esp_err_t error = read_schema3_record_slot(
        handle, SETTINGS_RECORD_SLOT_A, &slot_a, &slot_a_valid);
    if (error == ESP_OK) {
        error = read_schema3_record_slot(
            handle, SETTINGS_RECORD_SLOT_B, &slot_b, &slot_b_valid);
    }
    if (error != ESP_OK) {
        return error;
...
{
    if (maximum_schema >= 7U) {
        settings_record_t records[2] = {0};
        bool valid[2] = {false, false};
        const char *keys[] = {"cfg7_a", "cfg7_b"};
        for (size_t index = 0U; index < 2U; ++index) {
            uint8_t encoded[SETTINGS_RECORD_SCHEMA7_ENCODED_SIZE];
            size_t size = sizeof(encoded);
            const esp_err_t error = nvs_get_blob(handle, keys[index], encoded, &size);
            if (error == ESP_OK) {
                valid[index] = settings_record_decode_schema7(encoded, size, &records[index]);
            } else if (!recoverable_field_error(error) && error != ESP_ERR_NVS_INVALID_LENGTH) {
                return error;
            }
        }
        const settings_record_slot_t latest = settings_record_select_latest(
    ...
    settings_record_slot_t *active_slot, uint32_t *generation)
{
    ...
    if (!current_found) {
        *active_slot = SETTINGS_RECORD_SLOT_NONE;
        *generation = 0U;
        return recover_legacy_settings(handle, settings, 7U);
    }
    return ESP_OK;
}
                           ...
                           const app_settings_t *right)
{
    return left->schema_version == right->schema_version &&
           left->manual_saving_requested ==
               right->manual_saving_requested &&
...
esp_err_t app_settings_init(void)
{
    ...
    if (error == ESP_OK) {
        ...
        if (recoverable_field_error(error)) {
            ...
            needs_migration = error == ESP_OK;
        } else if (error == ESP_OK && schema >= 2U && schema <= 7U) {
            ...
            if (error == ESP_OK && current_found) {
                ...
                error = recover_legacy_settings(handle, &loaded, 7U);
                needs_migration = error == ESP_OK;
            }
        }
    }
    ...
    if (error == ESP_OK) {
        const int absolute_offset =
            loaded.utc_offset_minutes < 0
                ...
                : loaded.utc_offset_minutes;
        ESP_LOGI(TAG,
                 "settings ready: manual_saving=%s UTC%c%02d:%02d unit=%s volume=%u updates=%s alarm=%s %02u:%02u days=0x%02x",
                 loaded.manual_saving_requested ? "yes" : "no",
                 loaded.utc_offset_minutes < 0 ? '-' : '+',
...
esp_err_t app_settings_get(app_settings_t *settings)
{
    ...
    const esp_err_t error = app_settings_get_snapshot(&snapshot);
    if (error == ESP_OK) {
...
esp_err_t app_settings_toggle_manual_saving(bool *saved_requested)
{
    ...
    updated.manual_saving_requested =
        !updated.manual_saving_requested;
    const esp_err_t error = app_settings_validate(&updated)
