...
本文的下载与烧录示例以 v0.25.0 正式版为准；功能交互同时跟随当前源码。
...
| 文件 | 用途 | 安装方式 |
| --- | --- | --- |
| `esp32-rlcd-firmware-v0.25.0-factory.bin` | 首次安装、v0.6.0 及更早版本迁移、故障恢复 | ROM 下载模式写入 `0x0`，包含语音模型并清除 NVS |
| `esp32-rlcd-firmware-v0.25.0-ota.bin` | 已安装 v0.7.0+ 后的日常更新 | 在线更新、设置门户本地 OTA 或串行应用更新，保留 NVS |
| `esp32-rlcd-firmware-v0.25.0-model.bin` | 为旧设备补装自 v0.18.0 起提供的离线语音模型 | 仅与同版本 OTA 通过项目 USB 脚本写入，不可单独启动或上传网页 |

目标硬件为 Waveshare ESP32-S3-RLCD-4.2，Factory 镜像使用 DIO、80 MHz、16 MB Flash
参数；全部发布二进制的 SHA-256 见
[`dist/v0.25.0/SHA256SUMS`](../dist/v0.25.0/SHA256SUMS)。
...
v0.25.0 `-ota.bin`，或使用 USB 写入。v0.10.0 及更新版本可直接从 `ONLINE UPDATE`
...
直接写入 v0.25.0 Factory 即可同时安装应用和模型。
...
```bash
shasum -a 256 dist/v0.25.0/esp32-rlcd-firmware-v0.25.0-factory.bin
shasum -a 256 dist/v0.25.0/esp32-rlcd-firmware-v0.25.0-ota.bin
shasum -a 256 dist/v0.25.0/esp32-rlcd-firmware-v0.25.0-model.bin
```

Windows PowerShell 可执行：

```powershell
(Get-FileHash .\dist\v0.25.0\esp32-rlcd-firmware-v0.25.0-factory.bin -Algorithm SHA256).Hash.ToLower()
(Get-FileHash .\dist\v0.25.0\esp32-rlcd-firmware-v0.25.0-ota.bin -Algorithm SHA256).Hash.ToLower()
(Get-FileHash .\dist\v0.25.0\esp32-rlcd-firmware-v0.25.0-model.bin -Algorithm SHA256).Hash.ToLower()
Get-Content .\dist\v0.25.0\SHA256SUMS
```
...
py -m esptool --chip esp32s3 --port COM5 --baud 460800 `
  ...
  0x0 .\dist\v0.25.0\esp32-rlcd-firmware-v0.25.0-factory.bin
```
...
一次写入 v0.25.0 应用和同版本模型。离线语音模型自 v0.18.0 起提供。先把 OTA、模型和
`SHA256SUMS` 放在同一目录；从 Release 下载或使用仓库 `dist/v0.25.0/` 即已满足这一
条件。进入 ROM 下载模式后执行：
...
这通常表示设备从旧版本仅通过在线更新或网页 OTA 安装了 v0.25.0 应用。其他功能不受
