...
- 固件更新独立位于 `src/update/`：在线更新使用 HTTPS 清单、ESP-IDF CA 证书包和受控的
  ...
  清除 Wi-Fi 和本地 OTA。两种 OTA 路径共用双 OTA 分区与回滚。
- 音频驱动与诊断位于 `src/audio/`，使用板载 ES8311、ES7210、ESP-IDF I2S 和只保存于
  PSRAM 的非阻塞临时回放状态机。
- microSD 图片源使用 ESP-IDF 原生 1-bit SDMMC 和 FatFs，应用层只读扫描
  固定目录；PBM/BMP 单色图片校验与解码位于 `src/image/`。
...
仓库默认构建版本为 `0.13.0`。需要构建其他版本时，通过环境变量覆盖，不直接为一次
候选构建修改 `CMakeLists.txt`：

```bash
RLCD_PROJECT_VERSION=0.14.0-dev.1 ./scripts/build.sh
RLCD_PROJECT_VERSION=0.14.0 ./scripts/build.sh
```
...
- SemVer 不自动选择通道。稳定清单只允许 `0.13.0` 这类正式目标，测试清单用于
  `0.14.0-dev.1`、`0.14.0-rc.1` 等候选，也可在转正式期间指向正式目标。
...
5. 家庭 Wi-Fi 离线时首屏、RTC、月历、温湿度、按键和音频保持可用，不自动开放配网
   热点。`NORMAL` 的 1、5、15、60 分钟退避正确；Wi-Fi 已连接但 NTP 不可达时标记为
   不同失败阶段，同样不打断首屏。`SAVING` 不自动联网或重试；错误凭据只能通过用户
   明确执行重新配网来替换；
...
7. USB `RESET_WIFI` 与设置门户中的“清除 Wi-Fi 配置”都只清除项目网络命名空间，重启后
   回到首次配网；门户必须二次确认，且不会重置其他偏好；
...
9. “状态”页长按 `KEY` 2 秒触发即时校时；“音频”页长按 2 秒执行扬声器、双麦克风和
   最长 5 秒临时回放测试；“设置”页长按 3 秒开启设置门户。音频录制/回放中短按 `KEY`
   可停止、活动阶段短按 `BOOT` 可取消且清除 PSRAM 数据；其他页面长按 `KEY` 或运行时
   长按 `BOOT` 均无操作且不会误判为短按；
...
13. 设置门户可保存 `NORMAL/SAVING`、UTC 偏移、摄氏/华氏单位、播放音量和更新通道，
    可用手机时间写入 RTC，也可清除 Wi-Fi 配置；默认更新通道为稳定版，从 schema v1
    迁移到 v2 时原有偏好保持不变；单槽损坏后启动会读取完整槽并修复冗余，健康双槽启动
    不重复写入；保存和破坏性操作均须按页面说明确认；
14. `NORMAL` 显示秒并按 1 秒读取 RTC、5 秒读取环境、30 秒读取电池，同时允许自动校时、
    网络重试和版本检查；`SAVING` 不显示秒，以上周期分别为 60 秒、60 秒和 5 分钟，且不
    自动联网。两种模式的按键、音频、手动校时、设置门户和手动更新都可用，固件不得使用
    Light-sleep 或 Deep-sleep；
15. 设置门户能够从同一临时热点上传 `-ota.bin`；短按 `BOOT` 能在等待上传时安全取消，
    上传错误文件和中断传输不会改变当前启动槽，无互联网时仍可完成本地 OTA；五分钟截止
    后拒绝新请求，已开始的短设置事务在有限清理窗口内完整结束，长 OTA 到期后中止且不切槽；
16. 关机后按住 `BOOT` 再按 `PWR` 仍能正常进入 ROM 下载模式；此时应用未运行，屏幕不
    显示写入进度；
17. Factory 镜像重新烧录后 NVS 被清空并能重新完成首次配网；在线更新、设置门户本地
    OTA 和 `update-app.sh` 串行更新后 NVS、家庭 Wi-Fi 与设备偏好保留；
18. 空白 NVS 的配网二维码 60 秒后恢复离线首屏，临时热点在 5 分钟后关闭；RTC 无效时
    首屏稳定显示占位符，系统中心和其他本地功能仍可操作；
19. 串口和构建产物的字符串检查不包含实机家庭 SSID、家庭密码或二维码中的临时热点密码。
