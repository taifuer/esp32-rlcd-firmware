...
仓库默认构建版本为 `0.11.0`。需要构建其他版本时，通过环境变量覆盖，不直接为一次
候选构建修改 `CMakeLists.txt`：

```bash
RLCD_PROJECT_VERSION=0.12.0-dev.1 ./scripts/build.sh
RLCD_PROJECT_VERSION=0.12.0 ./scripts/build.sh
```
...
- SemVer 不自动选择通道。稳定清单只允许 `0.11.0` 这类正式目标，测试清单用于
  `0.12.0-dev.1`、`0.12.0-rc.1` 等候选，也可在转正式期间指向正式目标。
...
8. `BOOT` 只在首屏与月历之间切换，并能从任意系统页返回首屏；`KEY` 只进入并循环“状态、
   音频、设置、在线更新”四个系统页，所有次级页面 30 秒超时符合规范；
