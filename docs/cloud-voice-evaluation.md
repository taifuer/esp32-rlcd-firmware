...
本文记录 ESP32 RLCD Firmware 云端语音模型的选型依据。结论基于 2026-08-30 的阿里云
...
- 用户主动长按发起单轮会话，固件采用手动 PTT 和半双工播放；
...
| 当前固件交互 | 手动 PTT、单轮、半双工 | 手动 PTT、单轮、半双工 |
...
这些能力，也不会在一次按键会话结束后保留上下文，因此切换模型后不会自动变成连续双工
助手。
...
这只是便于比较的音频增量估算，不是账单承诺。系统提示、文本输入、历史上下文、实际回复
时长、地域、最低一秒取整、免费额度和优惠都会改变结果。当前固件每轮建立独立会话，不会
累积多轮历史费用；以后若保留上下文，必须重新评估费用上限。

## 目标板 A/B 方案

在相同 API Key 地域、Wi-Fi、扬声器音量和测试距离下，各模型至少重复 20 轮：

1. 使用固定语料覆盖中文、英文、中英混说、数字日期、单词解释与发音、短句、接近 10 秒
   的长句、无声和日常背景声；
2. 记录连接成功率、用户字幕完整率、回答正确性、首段字幕延迟、首段语音延迟、整轮耗时、
   取消是否及时以及失败后的下一轮恢复；
...
资源恢复与实际费用可接受时，才考虑把它改为默认模型。`smart_turn`、多轮上下文和
Function Calling 属于独立产品功能，需要另行设计回声处理、打断、屏幕交互、权限边界和
费用控制，不能与模型默认值变更一起顺带开启。
