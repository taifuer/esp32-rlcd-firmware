...
| 语音 | 本地安全指令，以及可选云端语音 Beta 的连接、转写与回复 | 按住 `KEY` 2 秒，松开后开始一次语音会话 |
...
“语音”页只在用户主动操作后采集。按住 `KEY` 2 秒进入会话，看到 `RELEASE KEY` 后
松开再开始说话；监听时短按 `KEY` 提前结束，连接、监听、识别、思考或播放阶段短按
`BOOT` 取消。设备不使用唤醒词，也不会在空闲时持续监听。

云端语音已开启、已保存 API Key、实际状态为 `NORMAL` 且家庭 Wi-Fi 当前取得 IPv4 时，
Beta 会直连阿里云百炼 Realtime，进行一次最长 10 秒的中英文语音问答，并在本页显示和
播放回复。默认使用
`qwen3-omni-flash-realtime`，高级设置可选择 `qwen-audio-3.0-realtime-flash`。家庭
Wi-Fi 离线、设备处于 `SAVING`、云端已停用或未配置 Key 时，本轮自动使用最长 5 秒的本地
MultiNet 指令识别，不会停在云端连接页等待网络。后端在一轮开始时确定，完成后重新长按才开始
下一轮；首版不跨会话保留多轮上下文。配置、上传数据、凭据和可能费用见
...
云端回复只展示和播放，绝不会作为设备命令执行。云端模式会把本轮 PCM 发送到阿里云百炼，
