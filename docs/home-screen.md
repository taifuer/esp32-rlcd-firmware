...
| 长按 `BOOT` | 无运行时操作 | 仅在“设置”页按住 2 秒切换省电策略；其他系统页无操作 |
| 长按 `KEY` | 图片页按住 2 秒进入当前图片的删除确认；其他日常页无操作 | 仅执行当前页面明确提示的操作 |
| `PWR` | 短按开机，长按关机 | 同左 |

月历、图片和所有系统页无操作 30 秒后返回首屏。长按中的按键、图片删除确认与执行、
音频测试、校时任务、设置门户和固件更新会暂停普通页面超时。图片删除确认页必须先松开
发起长按的 `KEY`，再短按 `KEY` 删除；短按 `BOOT` 或 10 秒无操作取消。音频录制或回放
时，屏幕明确显示的 `KEY: STOP` 与 `BOOT: CANCEL` 临时取代页面导航；在线更新检查、
连接和确认阶段也可按 `BOOT` 取消。操作结束后恢复表格中的普通语义。
...
- 实际 `NORMAL` 状态在中部以同一种 78 像素数字字体等大显示 `HH:MM:SS`；实际 `SAVING`
  状态显示 `HH:MM` 并按分钟刷新。默认 `AUTO` 策略按电量在两种状态间选择，两种布局的
  字形视觉中心都位于整屏中央；
...
| 设置 | 省电策略及当前状态、时区、温度单位、播放音量、离线闹钟和本地设置门户 | 按住 `BOOT` 2 秒切换省电策略；按住 `KEY` 3 秒开启设置门户 |
...
播放音量设为 `0%` 时，测试不会启用扬声器，仍会完成录音和双麦克风分析，提示音与回放
均不会播放，回放结果显示为 `NOT PLAYED`。
...
“设置”页显示电源策略及当前实际状态、时区、温度单位、播放音量和闹钟摘要。默认
`AUTO` 显示为 `AUTO > NORMAL` 或 `AUTO > SAVING`；手动策略只显示 `NORMAL` 或
`SAVING`。按住 `BOOT` 2 秒会按 `AUTO` → `NORMAL` → `SAVING` → `AUTO` 的固定顺序
选择下一种策略并保存，随后在原地应用，不需要开启热点；应用完成后显示成功，暂时无法
应用时显示等待重试。按住 `KEY` 3 秒仍会开启
最多 5 分钟的临时 WPA2 设置门户，屏幕显示标准 Wi-Fi 二维码、随机密码和
`192.168.4.1`：

若保存已经完成但运行态暂时无法切换，`POWER` 行显示目标策略与 `PENDING`，后台继续
重试；下一次设备端切换仍从屏幕所示的已保存策略继续。

![设置门户等待页](assets/settings-portal.svg)

手机页面可以选择“自动（推荐）”“始终正常”或“始终省电”，并修改 UTC 偏移、温度单位、
播放音量、单个每周闹钟和
更新通道，也可使用手机时间校准 RTC、恢复偏好默认值、清除 Wi-Fi 或上传本项目的 OTA
镜像。`AUTO` 冷启动首个有效电量不高于 20% 时进入 `SAVING`，否则进入 `NORMAL`；
运行期在不高于 20% 或不低于 25% 的一侧连续取得两次有效采样后才切换，无效读数保持
当前状态。连接 USB 数据主机时 `AUTO` 立即使用 `NORMAL`，不再检测到主机后以新鲜电量
读数重新判断。原板没有向 ESP32 提供 VBUS 或充电状态，因此普通充电器、充电宝和纯充电
线不能触发这项 USB 数据检测，仍按电量判断。保存策略立即生效，无需重启，也不会中断
已经开始的手动联网、OTA 或其他操作。
Beta 更新默认
...
- 仅首次配置或用户在设置门户清除网络配置后显示配网二维码；这是省电策略的例外，即使
