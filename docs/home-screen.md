...
```text
日常功能（BOOT）
  首屏 [-> 天气（已启用时）] [-> 市场看板（已启用时）] -> 月历
    [-> 图片（有效图片时）] [-> 音乐（有效歌曲时）] -> 对话 -> 循环
...
天气和市场看板开启后即加入对应页面，不要求首次获取成功；只有从 microSD 成功读到有效
图片或歌曲后才加入媒体页面。上述可选功能均未启用或不可用时，日常页面环为
“首屏 → 月历 → 对话 → 首屏”。
...
| 按键 | 日常页面 | 系统中心 |
| --- | --- | --- |
| 短按 `BOOT` | 切换到下一日常页；可选功能按当前状态加入 | 返回首屏 |
| 短按 `KEY` | 主界面、天气、市场看板、日历和空闲对话页进入“设置”；多图图片页切换下一张，单图进入“设置”；音乐页播放/暂停 | 切换到下一系统页 |
| 长按 `BOOT` | 音乐页按住 2 秒调音量，其余无操作 | 仅在“设置”页按住 2 秒切换手动提前省电；其他系统页无操作 |
| 长按 `KEY` | 天气或市场页按住 2 秒刷新；图片页按住 2 秒进入当前图片的删除确认；音乐页按住 2 秒下一首；对话页按住 2 秒发起会话 | 仅执行当前页面明确提示的操作 |
| `PWR` | 短按开机，长按关机 | 同左 |
...
| 页面或条件 | 页脚文案 |
| --- | --- |
| 天气 | 开启市场看板时 `BOOT: MARKET`，否则 `BOOT: CALENDAR`；`KEY: SETTINGS`；`HOLD KEY 2s: REFRESH` |
| 市场看板 | `BOOT: CALENDAR | KEY: SETTINGS`；`HOLD KEY 2s: REFRESH` |
| 月历（存在有效图片） | `BOOT: IMAGE | KEY: SETTINGS` |
| 月历（无有效图片） | 有音乐时 `BOOT: MUSIC`，否则 `BOOT: CHAT`；`KEY: SETTINGS` |
| 音乐 | `BOOT: CHAT`；`KEY: PLAY/PAUSE`；按住 KEY 下一首，按住 BOOT 调音量 |
| 图片（多图） | `BOOT: CHAT | KEY: NEXT IMAGE | 2/6`；第二行 `HOLD KEY 2s: DELETE IMAGE` |
| 图片（单图） | `BOOT: CHAT | KEY: SETTINGS`；第二行 `HOLD KEY 2s: DELETE IMAGE` |
| 状态 | `BOOT: HOME | KEY: SETTINGS | HOLD KEY 2s: SYNC TIME` |
| 对话（空闲或不可用） | `BOOT: HOME | KEY: SETTINGS` |
| 设置 | `BOOT: HOME | KEY: ONLINE UPDATE`；第二行按当前请求显示 `MANUAL SAVING ON` 或 `MANUAL SAVING OFF`，内容区提示 `HOLD KEY 2s TO OPEN` |
| 在线更新 | `BOOT: HOME | KEY: STATUS`；可操作时追加 `CHECK UPDATE` 或 `REVIEW UPDATE` |

所有有效长按在持续 1 秒后统一显示剩余时间与 `RELEASE TO CANCEL`；此时提前松开只取消
长按，不会再执行短按动作。v0.26.0 起，普通页面保持用户当前选择，直到手动切换，
不再因无操作返回时钟；正常开机仍显示时钟，不需要另设常驻页。
临时操作保留各自的时限，结束后回到所在功能页。图片删除确认页必须先松开
...
![市场看板效果图](assets/market-screen.svg)

看板默认关闭，开启后位于可选天气与月历之间。网页“联网 → 市场看板”可从 15 个预设中
选择并排序 1–5 项；设备显示点位、涨跌幅与各自的报价日期、时间，不增加设备配置菜单。

`NORMAL` 停留本页时约每 2 分钟获取；离页停止周期请求，`SAVING` 只在按住 `KEY`
2 秒主动刷新时临时联网。请求中与失败时保持表格，已有值及原报价时间继续显示，首次
无数据显示 `--`。顶部是 RTC 当前日期、时间；报价为数据源的 UTC+8 时间，不把请求时刻
冒充报价时间。无需 API Key 或 SD 卡，来源与限制见[市场看板 Beta](market-dashboard.md)。
...
存在有效图片时，月历页脚显示 `BOOT: IMAGE`；没有有效图片时有音乐则显示 `BOOT: MUSIC`，否则显示 `BOOT: CHAT`。
`KEY` 前往设置页，屏幕不会同时列出两个可能目标。

## microSD 图片

![microSD 图片页效果图](assets/image-screen.svg)

图片页是位于月历之后的可选日常页面。固件开机时使用 ESP-IDF 原生 1-bit SDMMC
...
`BOOT: CHAT | KEY: NEXT IMAGE | 2/6` 一类提示，单图时显示
`BOOT: CHAT | KEY: SETTINGS`；有音乐时下一目的地为 `MUSIC`。
两种情况都明确提示 `HOLD KEY 2s: DELETE IMAGE`。
...
microSD 不用于保存麦克风录音；用户歌曲放在独立的 `rlcd/music/` 目录。
必须物理关机后再插卡、拔卡或换卡；
...
对话属于日常功能页，位于可选音乐之后；未配网、未配置 AI 或模型缺失也保留入口。
短按 BOOT 返回时钟，短按 KEY 进入设置；进入空闲页不会停止后台音乐。

![AI 对话页效果图](assets/voice.svg)
...
三个系统页共用标题、`1/3`—`3/3` 位置、内容网格和底部按键提示，每页只负责一类问题：

| 页面 | 内容 | 当前页长按操作 |
| --- | --- | --- |
| 状态 | RTC 与备用电源、SHTC3、电池、最近时间同步结果和当前 Wi-Fi 链路 | 按住 `KEY` 2 秒立即校时 |
| 设置 | 网页入口与电源状态 | 按住 `BOOT` 2 秒切换手动提前省电；按住 `KEY` 2 秒直接打开网页 |
...
“设置”页只显示网页入口、实际电源状态及原因，保留手动提前省电快捷键。
...
`MANUAL SAVING OFF`，而不是重复当前状态。v0.28.0 按住 `KEY` 2 秒直接打开网页，不经过二级菜单。
完整按键、旧版区别、局域网授权与热点说明见[设备设置](device-settings.md)。
普通门户闲置 5 分钟关闭，操作可续期，一次最多 30 分钟。热点模式显示 Wi-Fi 二维码、
随机密码和 `192.168.4.1`；已连家庭网络时显示局域网地址与授权二维码：
...
也不自动退出。精简门户只提供已保存 Wi-Fi 的查看、更换、清除，以及本地 OTA；
