...
| 语音 | 本地安全指令，以及可选 AI 语音 Beta 的连接、转写、回复与续问 | 按住 `KEY` 2 秒，松开后开始一次语音会话 |
...
| 状态 | 语音 |
| :---: | :---: |
| ![状态页效果图](assets/status.svg) | ![AI 语音页效果图](assets/voice.svg) |
| 设置 | 在线更新 |
...
AI 语音 Beta 会直连阿里云百炼 Realtime，在同一 WebSocket 上保留服务端上下文，
最多进行 5 轮中英文问答。每轮语音输入最长 10 秒，回复后续问等待最长 30 秒，
新一轮只在会话开始后的 5 分钟内准入；已经开始的回答会完整播放。回复文字在本页按
实际字体宽度换行并持续显示最新 4 行。默认使用
