...
```text
日常功能（BOOT）
  首屏 [-> 天气（已启用时）] -> 月历 [-> 图片（有效图片时）] [-> 音乐（有效歌曲时）] -> 对话 -> 循环
...
图片页。天气、图片和音乐都不可用时，日常页面环为“首屏 → 月历 → 对话 → 首屏”。
...
| 短按 `KEY` | 主界面、天气、日历和空闲对话页进入“设置”；多图图片页切换下一张，单图进入“设置”；音乐页播放/暂停 | 切换到下一系统页 |
| 长按 `BOOT` | 音乐页按住 2 秒调音量，其余无操作 | 仅在“设置”页按住 2 秒切换手动提前省电；其他系统页无操作 |
| 长按 `KEY` | 天气页按住 2 秒刷新；图片页按住 2 秒进入当前图片的删除确认；音乐页按住 2 秒下一首；对话页按住 2 秒发起会话 | 仅执行当前页面明确提示的操作 |
| `PWR` | 短按开机，长按关机 | 同左 |
...
| 页面或条件 | 页脚文案 |
| --- | --- |
| 天气 | `BOOT: CALENDAR | KEY: SETTINGS | HOLD KEY 2s: REFRESH` |
