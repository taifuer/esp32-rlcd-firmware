...
```text
日常功能（BOOT）
  首屏 [-> 天气（已启用时）] -> 月历 [-> 图片（有效图片时）] [-> 音乐（有效歌曲时）] -> 循环
...
图片页。天气、图片和音乐都不可用时，日常页面环为“首屏 → 月历 → 首屏”。
...
| 短按 `KEY` | 主界面、天气和日历进入“对话”；多图图片页切换下一张，单图进入“对话”；音乐页播放/暂停 | 切换到下一系统页 |
| 长按 `BOOT` | 音乐页按住 2 秒调音量，其余无操作 | 仅在“设置”页按住 2 秒切换手动提前省电；其他系统页无操作 |
| 长按 `KEY` | 天气页按住 2 秒刷新；图片页按住 2 秒进入当前图片的删除确认；音乐页按住 2 秒下一首 | 仅执行当前页面明确提示的操作 |
| `PWR` | 短按开机，长按关机 | 同左 |
...
| 页面或条件 | 页脚文案 |
| --- | --- |
| 天气 | `BOOT: CALENDAR | KEY: CHAT | HOLD KEY 2s: REFRESH` |
| 月历（存在有效图片） | `BOOT: IMAGE | KEY: CHAT` |
| 月历（无有效图片） | 有音乐时 `BOOT: MUSIC`，否则 `BOOT: HOME`；`KEY: CHAT` |
| 音乐 | `BOOT: HOME`；`KEY: PLAY/PAUSE`；按住 KEY 下一首，按住 BOOT 调音量 |
| 图片（多图） | `BOOT: HOME | KEY: NEXT IMAGE | 2/6`；第二行 `HOLD KEY 2s: DELETE IMAGE` |
| 图片（单图） | `BOOT: HOME | KEY: CHAT`；第二行 `HOLD KEY 2s: DELETE IMAGE` |
...
`BOOT: HOME | KEY: CHAT`；两种情况都明确提示 `HOLD KEY 2s: DELETE IMAGE`。
...
| 设置 | 实际电源状态与原因、时区、温标、播放音量和离线闹钟 | 按住 `BOOT` 2 秒切换手动提前省电；按住 `KEY` 2 秒进入快捷设置，其中保留网页入口 |
...
`MANUAL SAVING OFF`，而不是重复当前状态。v0.27.0 候选按住 `KEY` 2 秒进入
`QUICK SETTINGS`，直接修改音量与闹钟开关，也可选择 `WEB SETTINGS`
