...
仓库中的 v0.23.0 正式发布目录可以直接使用：

```bash
./scripts/update-app.sh \
  --port COM5 \
  --firmware dist/v0.23.0/esp32-rlcd-firmware-v0.23.0-ota.bin \
  --model dist/v0.23.0/esp32-rlcd-firmware-v0.23.0-model.bin \
  --confirm
```
...
旧设备迁移到 v0.23.0 且首次启用自 v0.18.0 起提供的离线语音时，需要执行一次本流程来
...
仓库默认生成 `0.23.0`。构建后续预发布候选时使用环境变量，例如：
