...
rlcd_firmware_ota.bin      # 在线更新、本地更新或保留配置的串行应用更新
```
...
普通用户和日常开发优先使用设备 `LOCAL UPDATE` 页提供的本地更新，无需数据线和 ROM
下载模式。完整步骤见[固件安装与更新](firmware-update.md)。

### 构建版本与在线通道

仓库默认生成 `0.10.0`。构建预发布候选时使用环境变量，例如：

```bash
RLCD_PROJECT_VERSION=0.10.0-rc.1 ./scripts/build.sh
```

带 `-dev`、`-rc.1` 等预发布标识的版本只读取 `testing.json`；没有预发布标识的正式版本
只读取 `stable.json`。不要为了测试临时修改正式清单，也不要把前导 `v` 写进
`RLCD_PROJECT_VERSION`。

v0.9.0 没有在线更新客户端。首次迁移到 v0.10.0 仍需从 v0.9.0 系统中心最后一页的本地
更新入口上传 `build/rlcd_firmware_ota.bin`，或按本页的串行应用更新流程写入。设备运行
带预发布标识的构建时，可通过 `testing.json` 验证后续预发布版本的在线更新。
...
RTC 写后回读和错误密码恢复；验证 `BOOT` 的日常页面路径、`KEY` 的六个系统页面、网络页
2 秒手动校时，以及 Wi-Fi 维护页 5 秒前取消和满 5 秒重配网。USB `RESET_WIFI` 也应只
清除项目配置。

在线更新还需用两个递增的预发布版本验证完整路径：第一次通过本地更新或串行方式安装，
...
确认和回滚。任何失败都不得改变当前可启动槽，本地更新在无互联网时仍可用。
