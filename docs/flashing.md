...
旧设备迁移到 v0.27.0 且首次启用自 v0.18.0 起提供的离线语音时，需要执行一次本流程来
...
仓库默认生成 `0.27.0`。构建后续预发布候选时使用环境变量，例如：

```bash
RLCD_PROJECT_VERSION=0.28.0-dev.1 ./scripts/build.sh
```
