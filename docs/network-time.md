...
未保存 Wi-Fi ──> 临时加密热点与配网页面 ──> 凭据写入 NVS
      │                                       │
      └─ 5 分钟无人配置 ─> 关闭 Wi-Fi        v
                                             NORMAL ─> 家庭 Wi-Fi ─> SNTP ─> RTC
                                                │          │失败            │
                                                │          └─ 退避重试      └─ 检查更新后关闭 Wi-Fi
                                                └─ 每 24 小时重新同步

                                             SAVING ─> 保持 Wi-Fi 关闭，等待用户发起网络操作
...
6. 校时成功后，设备按当前时区写入 PCF85063 RTC；默认时区为 UTC+8。`NORMAL` 模式会
   检查对应通道的版本清单，完成后关闭 Wi-Fi，并在运行期间每 24 小时重新同步；
   `SAVING` 模式不自动联网、重试或检查更新，用户仍可手动校时和检查更新。

`NORMAL` 的自动更新流程只检查并记录 `ONLINE UPDATE` 页的结果，不会下载、安装、切换
...
中，因此 `NORMAL` 模式再次开机后会自动联网并恢复时间，不再需要每次连接 USB 手动
校时；`SAVING` 模式等待用户从“状态”页手动校时。
...
- `NORMAL` 的自动连接、SNTP 与后台重试不使用全屏状态页；无论 RTC 是否有效，日常页面
  均保持可用；`SAVING` 不执行这些后台联网操作；
...
确认后，设备只清除 `rlcd_net` 命名空间并重启进入首次配网。该功能与 USB
`RESET_WIFI` 清除范围相同，不会擦除整片 Flash，也不会重置其他偏好。
...
家庭 Wi-Fi 或 NTP，在 `NORMAL` 与 `SAVING` 下都可使用。
...
时可从设置门户上传 OTA 固件，RTC 与其他离线功能不受影响。
...
- `GET_AUDIO` 返回 codec 就绪、测试阶段、录制时长、回放来源和两路强度百分比，不输出
  原始音频；
- `GET_SETTINGS` 返回当前运行模式、UTC 偏移、温度单位、回放音量和更新通道，不修改设置；
- `RESET_WIFI` 是开发和恢复入口，只清除本项目的网络凭据命名空间，然后重启进入配网
  模式；日常使用优先从设置门户执行相同操作；
...
- 家庭 Wi-Fi 凭据、设备偏好和 RTC 备用电池判定分别保存在独立的 NVS 命名空间；设置
  门户清除 Wi-Fi 不会重置电源模式、时区、温度单位、音量或 Beta 更新偏好；设备偏好
