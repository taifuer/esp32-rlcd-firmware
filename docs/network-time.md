...
`testing.json`。在线检查、在线安装、云端语音、设置门户、本地 OTA 和网络校时由同一个
...
- 可选云端语音 API Key 位于独立的 `rlcd_cloud` NVS 命名空间，同样不具备物理读取保护；
  状态接口和日志不返回 Key，但用户仍应使用专用最小权限 Key，并在设备遗失、转让或送修前
  清除配置、吊销旧 Key。完整说明见[云端语音 Beta](cloud-voice.md)；
...
- 家庭 Wi-Fi 凭据、云端语音配置、设备偏好、闹钟已触发记录和 RTC 备用电池判定分别保存
