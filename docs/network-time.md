...
`RESET_WIFI` 的清除范围相同，不会擦除整片 Flash。

设备正常运行时按住 `BOOT` 2 秒，可立即请求一次联网校时。请求会唤醒原有网络任务，
复用已经保存在 NVS 的家庭 Wi-Fi 凭据和同一组 NTP 服务器；成功写入并回读 RTC 后再次
关闭 Wi-Fi。该操作不会显示或修改 Wi-Fi 凭据。未配置网络、正在配网或网络任务已经执行
其他操作时会直接提示不可用。

`BOOT` 仍保留 ESP32-S3 的硬件下载模式：只有关机后按住 `BOOT` 再按 `PWR` 才会进入 ROM
下载器。运行时短按/长按由应用处理，不会自行重启或进入下载模式。
