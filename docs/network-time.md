...
已保存 Wi-Fi ─> 设置门户显示网络名称 ─> 内存中验证新凭据
                                               ├─ 取得 IPv4 ─> 写入 NVS 并按实际状态恢复
                                               └─ 失败 ───> 保留旧凭据与门户
...
4. 临时热点最多开放 5 分钟；屏幕显示配网二维码 60 秒后恢复离线首屏，热点在剩余窗口内
   ...
   关闭 Wi-Fi，需要配网时可重新开机。已有网络需要更换时，从“设置”页开启设置门户，
   直接填写并验证新网络，不需要先清除旧配置。
...
6. 校时成功后，设备按当前时区写入 PCF85063 RTC；默认时区为 UTC+8。实际 `NORMAL`
   状态会保持家庭 Wi-Fi 连接、检查对应通道的版本清单，并在运行期间每 24 小时重新同步；
   实际 `SAVING` 状态不自动联网、重试或检查更新，用户手动校时、检查更新或下载图库时
   才临时连接，操作结束后关闭无线。

`NORMAL` 的常连是保持与家庭 AP 的关联和可用 IPv4，不是让射频持续满功率工作。固件使用
ESP-IDF `WIFI_PS_MIN_MODEM`：空闲时按 AP 的 DTIM 周期进入 Modem-sleep，需要收发数据时
自动唤醒。它兼顾后续联网功能的即时可用性与关联空闲功耗，不新增独立 Wi-Fi 开关。
...
IPv4 时显示 Wi-Fi 图标；未连接、连接中、失败、配网 SoftAP 和 `SAVING` 空闲时均留空。
图标不表示互联网或 NTP 一定可用，因此已经取得 IPv4 但 NTP 不可达时仍会显示。“状态”页
用独立的 `TIME SYNC` 和 `WI-FI` 行分别表达最近同步结果与当前链路。RTC 时间无效时，首屏
按当前模式使用 `--:--:--` 或 `--:--`，并显示日期占位符；其余本地功能继续运行，也可以
通过 USB 手动校时。
...
- 实际 `NORMAL` 状态的自动连接、常驻 STA、SNTP 与后台重试不使用全屏状态页；无论 RTC
  是否有效，日常页面均保持可用；实际 `SAVING` 状态不执行这些后台联网操作；
...
实际 `NORMAL` 状态下，已保存的网络连接失败时，单次尝试最多等待约 40 秒；家庭 Wi-Fi
已取得 IPv4 但 NTP 不可达时，单次同步最多等待约 20 秒，STA 仍保持连接。两者都会保留
凭据和 RTC，且不自动开放配网热点：连续失败后分别等待 1、5、15 分钟，之后保持每 15
分钟尝试一次。网络恢复后正常同步并重置退避级别。实际 `SAVING` 状态不执行自动重试。
两种状态都只在用户明确操作后替换错误凭据，避免临时断网或仅 NTP 不可用时周期性打断
首屏。

需要主动更换家庭 Wi-Fi 时，短按 `KEY` 进入系统中心，切换到“设置”页并按住 `KEY`
3 秒。连接屏幕所示设置热点、打开 `http://192.168.4.1`：

- 门户使用临时 SoftAP，期间暂停家庭 STA；页面显示的是 NVS 中的“已保存网络”，不是
  当前正在连接的网络。保存的 SSID 可以显示，密码永不回传、预填或写入日志；
- 填写新 SSID 和密码后，设备只在易失内存中保留候选凭据并尝试连接。只有取得 IPv4
  才会将新凭据写入 `rlcd_net` NVS；
- 验证失败时已保存的凭据不变，设置门户继续可用，可修正后重试。验证过程中 SoftAP
  可能因无线信道切换让手机短暂断开，此时重新连接屏幕所示热点并打开同一地址即可；
- 验证成功后设备保存新凭据并关闭设置门户；实际 `NORMAL` 随后连接新网络、校时并保持
  在线，实际 `SAVING` 完成本次主动校时后关闭无线。RTC、屏幕和其他本地任务不中断，
  整机不重启。未修改网络时直接关闭门户也按同一规则恢复。

“清除 Wi-Fi”是另一个需要再次确认的忘记网络操作，不是更换网络的必经步骤。确认后，
设备只清除 `rlcd_net` 命名空间，关闭设置热点并在不重启的情况下进入首次配网。手机会因热点切换
断开一次，需要按屏幕二维码重新连接。该功能与 USB `RESET_WIFI` 清除范围相同，不会擦除整片
Flash，也不会重置其他偏好。

需要立即联网校时时，在系统中心切换到“状态”页并按住 `KEY` 2 秒。请求会唤醒原有
网络任务，复用 NVS 中的家庭 Wi-Fi 凭据和同一组 NTP 服务器；成功写入并回读 RTC 后按
实际状态收尾：`NORMAL` 保持连接，`SAVING` 关闭 Wi-Fi。该操作不会显示或修改凭据；
处于离线退避时也可立即重试。未配置网络、正在配网或网络任务忙碌时会直接提示不可用。
...
中的家庭 Wi-Fi 凭据：`NORMAL` 复用已有连接，`SAVING` 建立有时限的临时 HTTPS 会话；
所有版本默认读取 `stable.json`，只有用户在设置门户主动开启 Beta 更新后才读取
`testing.json`。在线检查、在线安装、AI 对话、设置门户、本地 OTA 和网络校时由同一个
会话策略串行执行，避免多个任务同时控制 Wi-Fi。没有互联网时可从设置门户上传 OTA 固件，RTC
与其他离线功能不受影响。手动省电请求或实际状态在运行中切换不重启设备，也不会取消
已经开始的手动联网、在线检查、OTA 或本地操作；事务结束后再按最终实际状态保持或关闭
连接。
...
- `GET_NETWORK` 只返回状态、是否已配置、当前 STA 是否取得 IPv4、自动联网策略、最近
  失败阶段（`wifi`、`ntp` 或 `service`）及最近错误，不输出家庭 Wi-Fi 名称或密码；
...
- 家庭 Wi-Fi 名称和密码只写入设备的 `rlcd_net` NVS 命名空间；设置门户可返回已保存的
  SSID，但密码不回传、预填或记录。源码、构建目录、串口日志和 Git 仓库均不保存实机凭据；
- 更换网络时，候选凭据只在 RAM 中保留，必须取得 IPv4 才写入 NVS；验证失败保留原凭据，
  不使用“先删除、再重配”作为日常更换流程；
- 当前分区未启用 NVS 加密，因此能够物理读取 Flash 的攻击者可能取得网络凭据；这与面向
  低风险家庭环境的初始版本定位一致，不应把设备交给不可信人员；
- 可选 AI 对话 API Key 位于独立的 `rlcd_cloud` NVS 命名空间，同样不具备物理读取保护；
  状态接口和日志不返回 Key，但用户仍应使用专用最小权限 Key，并在设备遗失、转让或送修前
  清除配置、吊销旧 Key。完整说明见[AI 对话 Beta](cloud-voice.md)；
...
- 家庭 Wi-Fi 凭据、AI 对话配置、设备偏好、闹钟已触发记录和 RTC 备用电池判定分别保存
  在独立的 NVS 命名空间；设置门户清除 Wi-Fi 不会重置手动提前省电、时区、温度单位、
  音量或 Beta 更新偏好；设备偏好
...
- 本项目的 Factory 镜像从 `0x0` 写入，映像中的空白区会覆盖位于 `0x9000` 的 NVS
  ...
  OTA 和 `update-app.sh` 串行 app+model 更新不会覆盖 NVS，正常更新无需重新配置；
...
更新、设置门户、本地 OTA 与校时任务必须先取得独占网络会话，避免 HTTP 服务、STA 和
SoftAP 模式相互竞争。实际 `NORMAL` 的空闲 STA 可以在事务之间保持连接并被后续任务
复用；实际 `SAVING` 的临时会话释放后停止 Wi-Fi。
