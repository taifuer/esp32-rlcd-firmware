...
6. 校时成功后，设备按中国标准时间（UTC+8、无夏令时）写入 PCF85063 RTC，并关闭
...
完整安装会清除判定记录，本地 OTA 和 Wi-Fi 重置会保留它。
...
- 本项目的 Factory 镜像从 `0x0` 写入，映像中的空白区会覆盖位于 `0x9000` 的 NVS
  分区。因此首次安装、分区迁移或完整恢复会清除 Wi-Fi 配置；本地网页 OTA 和
  `update-app.sh` 串行应用更新不会覆盖 NVS，正常升级无需重新配网；
...
DHCP Option 114 提示和固定地址，不复制第三方 DNS 重定向服务源码。固件升级会先取得
网络维护窗口，在临时升级热点关闭后才允许后台 SNTP 任务继续使用 Wi-Fi，避免两个本地
HTTP 服务或无线模式相互竞争。
