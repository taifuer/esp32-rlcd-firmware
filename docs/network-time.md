...
未保存 Wi-Fi ──> 临时加密热点与配网页面 ──> 凭据写入 NVS
      │                                       │
      └─ 5 分钟无人配置 ─> 关闭 Wi-Fi        v
                                  后台连接家庭 Wi-Fi ─> SNTP ─> 写入 PCF85063
                                          │                     │
                                          │失败                 └─ 自动检查更新后关闭 Wi-Fi，
                                          v                        24 小时后重校
                              关闭 Wi-Fi，按 1/5/15/60 分钟退避重试
...
4. 临时热点最多开放 5 分钟；屏幕显示配网二维码 60 秒后恢复离线首屏，热点在剩余窗口内
   ...
   关闭 Wi-Fi，需要配网时可重新开机，或在“Wi-Fi 维护”页执行明确的重新配网操作。
5. 设备关闭临时热点，连接家庭 Wi-Fi，并配置 `ntp.aliyun.com`、
   `cn.pool.ntp.org` 和 `pool.ntp.org`。
6. 校时成功后，设备按中国标准时间（UTC+8、无夏令时）写入 PCF85063 RTC，再短暂连接
   固件服务检查对应通道的版本清单，完成后关闭 Wi-Fi；运行期间每 24 小时重新同步一次。

自动流程只检查更新并记录 `ONLINE UPDATE` 页的结果，不会下载、安装、切换页面或打断
首屏。安装必须由用户在“在线更新”页先按住 `KEY` 2 秒进入 `REVIEW`，再在确认页按住
`KEY` 3 秒明确确认。检查失败不影响已经写入的 RTC，也不会阻止本地功能。
...
的网络结果图标。RTC 时间无效时，首屏使用 `--:--:--` 和日期占位符，其余本地功能继续
运行，也可以通过 USB 手动校时。

### RTC 备用电池保持判定

PCF85063 的供电网络没有连接 ESP32 ADC，因此固件不能直接读取备用电池电压、容量或是否
插入。设备健康页采用跨断电验证：第一次取得有效 RTC 时间时，在独立的 `rlcd_rtc` NVS
...
完整安装会清除判定记录，在线更新、本地更新和 Wi-Fi 重置会保留它。
...
- 自动连接、SNTP 与后台重试不使用全屏状态页；无论 RTC 是否有效，日常页面均保持可用；
- RTC 备用电池判定在任何 Wi-Fi 或 NTP 操作前完成，联网恢复时间不会改写本次判定结果；
- 仅在没有保存网络配置时显示配网信息 60 秒，随后恢复仪表盘；临时热点与配网页面最多
  保持 5 分钟；按 `BOOT: OFFLINE` 可立即返回仪表盘，需要重新查看密码或重新开放热点时
  可重启设备；
- 配网和错误页是静态画面，固件不会每秒重复刷新全反射屏。

已保存的网络连接失败时，固件最多等待约 40 秒并关闭 Wi-Fi；家庭 Wi-Fi 已连接但 NTP
不可达时，最多等待约 20 秒后关闭 Wi-Fi。两者都会保留凭据和 RTC，且不自动开放配网
热点：连续失败后分别等待 1、5、15、60 分钟，之后保持每 60 分钟尝试一次。网络恢复后
正常同步并重置退避级别。需要修改错误凭据时，由用户在“Wi-Fi 维护”页明确执行重新配网，
避免临时断网或仅 NTP 不可用时周期性打断首屏。

需要主动更换家庭 Wi-Fi 时，短按 `KEY` 进入系统中心，再切换到“Wi-Fi 维护”页。只有在
该页按住 `KEY` 才会启动确认：1 秒后显示倒计时，满 5 秒后清除 `rlcd_net` 命名空间并
重启进入首次配网；提前松开会取消。该功能与 USB `RESET_WIFI` 清除范围相同，不会擦除
整片 Flash。

需要立即校时时，在系统中心切换到“网络与时间”页并按住 `KEY` 2 秒。请求会唤醒原有
...
中的家庭 Wi-Fi 凭据建立有时限的 HTTPS 会话；正式版读取 `stable.json`，带预发布标识的
开发版读取 `testing.json`。在线检查、在线安装、本地更新和网络校时由同一个会话策略
串行执行，避免多个任务同时控制 Wi-Fi。没有互联网时改用“本地更新”，RTC 与其他离线
功能不受影响。
...
- 本项目的 Factory 镜像从 `0x0` 写入，映像中的空白区会覆盖位于 `0x9000` 的 NVS
  分区。因此首次安装、分区迁移或完整恢复会清除 Wi-Fi 配置；在线更新、本地更新和
  `update-app.sh` 串行应用更新不会覆盖 NVS，正常更新无需重新配网；
...
更新、本地更新与校时任务必须先取得独占网络会话；当前任务释放 Wi-Fi 后，下一个任务才
能继续，避免 HTTP 服务、STA 和 SoftAP 模式相互竞争。
