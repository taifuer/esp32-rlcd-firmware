...
直接使用本地 MultiNet，不等待云端连接超时。此时页面显示 `OFFLINE COMMANDS` 和
`Hold KEY 2s for a command`，明确它只接受设备指令，不把本地识别描述为自由问答。
...
1. `AI CHAT` 空闲页显示 `Hold KEY 2s to ask`；按住 `KEY` 2 秒；
2. 看到提示后松开 `KEY`，再开始说话；
3. 单次输入最长 10 秒，讲话结束后可短按 `KEY` 提前结束；
4. 思考和播放期间等待回复；轮次未满时，播放页显示 `KEY: NEXT TURN`，短按 `KEY`
   可跳过剩余回复并
